<?php
class ControllerSaleMailsubscribeScheduler extends Controller {
	public function index(){
		$this->load->language('sale/mailsubscribe_scheduler');
		
		$this->document->setTitle($this->language->get('heading_title'));
		
		$this->load->model('sale/mailsubscribe_scheduler');
	...
	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->model_sale_mailsubscribe_scheduler->saveSchedule($this->request->post);
			...
			$this->response->redirect($this->url->link('sale/mailsubscribe_scheduler', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		}
		$this->getForm();
	}
	public function update() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
      	  	$this->model_sale_mailsubscribe_scheduler->saveSchedule(array_merge($this->request->post, array('schedule_id' => $this->request->get['schedule_id'])));
			...
			$this->response->redirect($this->url->link('sale/mailsubscribe_scheduler', 'token=' . $this->session->data['token'], 'SSL'));
		}
		$this->getForm();
	}
	public function delete(){
		...
		if (isset($this->request->post['selected']) && ($this->validateDelete())) {
			foreach ($this->request->post['selected'] as $schedule_id) {
				$this->model_sale_mailsubscribe_scheduler->deleteSchedule($schedule_id);
	...
	private function getList(){
		...
		$data['column_recurring'] = $this->language->get('column_recurring');
		...
		if (isset($this->session->data['success'])) {
  		...
  		$data['breadcrumbs'] = array();
   		...
   		$data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('sale/mailsubscribe_scheduler', 'token=' . $this->session->data['token'], 'SSL'),
      		'separator' => ' :: '
   		);
		$data['insert'] = $this->url->link('sale/mailsubscribe_scheduler/insert', 'token=' . $this->session->data['token'], 'SSL');		
    	$data['delete'] = $this->url->link('sale/mailsubscribe_scheduler/delete', 'token=' . $this->session->data['token'] .$url, 'SSL');
		...
		$schedules_total = $this->model_sale_mailsubscribe_scheduler->getSchedulesTotal($data2);
		
		$results = $this->model_sale_mailsubscribe_scheduler->getSchedules($data2);
    	...
    	foreach ($results as $result) {
			...
			/*$action[] = array(
				...
				'href' => $this->url->link('sale/mailsubscribe_scheduler/info', 'token=' . $this->session->data['token'] . '&schedule_id=' . $result['schedule_id'], 'SSL')
			);*/
			
			$action[] = array(
				'text' => $this->language->get('text_edit'),
				'href' => $this->url->link('sale/mailsubscribe_scheduler/update', 'token=' . $this->session->data['token'] . '&schedule_id=' . $result['schedule_id'], 'SSL')
			);
			...
			$data['schedules'][] = array(
				...
				'recurrent'   	=>(($result['recurrent'])?$this->language->get('text_yes'):$this->language->get('text_no')),
		...
		$pagination->total = $schedules_total;
		$pagination->page = $page;
		$pagination->limit = $this->config->get('config_limit_admin');
		$pagination->url = $this->url->link('sale/mailsubscribe_scheduler', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		
		$data['pagination'] = $pagination->render();
		
		$data['results'] = sprintf($this->language->get('text_pagination'), ($schedules_total) ? (($page - 1) * $this->config->get('config_limit_admin')) + 1 : 0, ((($page - 1) * $this->config->get('config_limit_admin')) > ($schedules_total - $this->config->get('config_limit_admin'))) ? $schedules_total : ((($page - 1) * $this->config->get('config_limit_admin')) + $this->config->get('config_limit_admin')), $schedules_total, ceil($schedules_total / $this->config->get('config_limit_admin')));
		...
		$this->response->setOutput($this->load->view('sale/mailsubscribe_scheduler.tpl', $data));	
	}
	private function getForm() {
		
		if (isset($this->request->get['schedule_id'])) {
            $schedule_id = $this->request->get['schedule_id'];
            $schedule_info = $this->model_sale_mailsubscribe_scheduler->getSchedule($schedule_id);
        } else {
            $schedule_info = array();
        }
		/*echo '<pre>';
		...
		if (!isset($schedule_id)) {
            $data['action'] = $this->url->link('sale/mailsubscribe_scheduler/insert', 'token=' . $this->session->data['token'], 'SSL');
        } else {
            $data['action'] = $this->url->link('sale/mailsubscribe_scheduler/update', 'token=' . $this->session->data['token'] . '&schedule_id=' . $schedule_id, 'SSL');
        }
				
    	$data['cancel'] = $this->url->link('sale/mailsubscribe_scheduler', 'token=' . $this->session->data['token'], 'SSL');
        ...
        /*changing here for random products in the schedular for data
		...
		if (isset($this->request->post['defined_product_text'])) {
            ...
            $data['defined_product_text'] = $schedule_info['defined_product_text'];
        } else {
        ...
        if (isset($this->request->post['defined_categories'])) {
            ...
            $data['defined_categories'] = $schedule_info['defined_categories'];
        } else {
        ...
        if (isset($this->request->post['defined_category'])) {
            ...
            $data['defined_category'] = unserialize($schedule_info['defined_category']);
        } else {
		...
		if (isset($this->request->post['defined_manufactures'])) {////for radio button
            ...
            $data['defined_manufactures'] = $schedule_info['defined_manufactures'];
        } else {
            $data['defined_manufactures'] = false;
        }
		
		if (isset($this->request->post['defined_manufacture'])) {///for array of manufacture
            $data['defined_manufacture'] = $this->request->post['defined_manufacture'];
        } elseif (!empty($schedule_info)) {
            $data['defined_manufacture'] = unserialize($schedule_info['defined_manufacture']);
        } else {
		...
		$data['manufactures'] = $this->model_catalog_manufacturer->getManufacturers($data2); 
		...
		$this->response->setOutput($this->load->view('sale/mailsubscribe_scheduler_form.tpl', $data));	
	}
	protected function validateDelete() {
    	if (!$this->user->hasPermission('modify', 'sale/mailsubscribe_scheduler')) {
